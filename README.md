...
- **Deal detection** - 3 independent signals, shown separately rather than blended
  into one opaque score
- **Price history** - every observation is retained, so an item's price can be judged
  against its own past rather than against a marketing claim
...
**Flipp backend API** - live cross-store pricing, pulled from two arrays in the same
response: `ecom_items[]` (online listings) and `items[]` (weekly flyer/print-ad listings,
stored as `source='flyer'`). The two arrays cover almost entirely different retailers.
`ecom_items[]` surfaces Walmart plus national drugstore/office/hardware retailers (Shoppers
Drug Mart, London Drugs, Well.ca, Healthy Planet, RONA, Staples, Bureau en gros, Best Buy) —
9 merchants total. `items[]` is where the big grocery banners actually live: Metro, Sobeys,
Food Basics, Real Canadian Superstore, Loblaws, No Frills, FreshCo, Fortinos, Longos, Farm
Boy, Your Independent Grocer, plus a long tail of regional grocers. Adding flyer ingestion
...
province. Used as a historical baseline.
...
Walmart plus a mix of drugstore, office-supply, and hardware retailers — not the big grocery
banners that blocked direct scraping. That turned out to be an endpoint choice, not a Flipp
...
than showing a stale price — verified by inserting a same-item observation with an expired
...
One more thing the flyer data surfaces that ecom never did: some flyer entries are
...
*several* products at once as a single ad slot —
...
- **Coverage is limited to the tracked basket** of 24 common grocery search terms (milk,
  ...
  flour, sugar, canned beans, frozen vegetables — see `ingest.py`), since the upstream API is
